...
const Screen = () => {
    ...
    useEffect(() =>{
        let scrollbar =  Scrollbar.init(document.querySelector('#mainScreen')!, scrollOption);
      ...
      <div className="main">
            ...
            <div className="imgContainer">
                {
                    duckData.map((item: any, index: any) => {
                        ...
                        return <DuckCard key={item.id} img={img} number={item.id} attribute = {item} />;
                    })
                }
            </div>
        </div>
        <div className="control">
          ...
          <div className="buttons">
            <button className="gridButton" onClick={() =>  setCurrentGrid('3x')} >3x3</button>
            <button className="gridButton" onClick={() =>  setCurrentGrid('4x')} >4x4</button>
          </div>
        </div>
      </div>
      <div className="sideControl">
        <button className="sideButton" onClick={sideControlUp}></button>
        <button className="sideButton down"  onClick={sideControlDown} ></button>
      </div>
    </div>
  );
};
